...
export class AccountRepository extends Repository<AccountEntity> {
    ...
    async updateAccount(updateAccountDto: UpdateAccountDto, id: number): Promise<AccountDto> {
        try {
            const { password, id } = updateAccountDto;
            const passwordHash = await this.hashPassword(password, jwtConfig.SALT);
            const accountUpdate = await getConnection()
                .createQueryBuilder()
                .update(AccountEntity)
                .set({password: passwordHash, otpassword: ''})
                .where('id = :id', {id})
                .execute();

            return plainToClass(AccountDto, accountUpdate);
        } catch (error) {
            this.logger.error(error.message, error.stack);
