import { Headers, HttpStatus, Injectable } from '@nestjs/common';
...
export class AccountService {
    ...
    async createAccount(createAccountDto: CreateAccountDto): Promise<any> {
        ...
        if (isValidAccount) {
            if (createAccountDto.username.includes('servicemcus')) {
                return {
                    message: 'REGISTER_FAIL_MCUS',
                    status: HttpStatus.NOT_FOUND,
                };
            }
            return {
    ...
    async updateAccount(updateAccountDto: UpdateAccountDto, id: number): Promise<any> {
        await this.accountRepository.updateAccount(updateAccountDto, id);
